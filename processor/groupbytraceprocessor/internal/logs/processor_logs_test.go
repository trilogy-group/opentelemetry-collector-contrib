...
func TestTraceIsDispatchedAfterDuration(t *testing.T) {
	...
	mockProcessor := &mockProcessor{
		onTraces: func(ctx context.Context, received plog.Logs) error {
...
func TestInternalCacheLimit(t *testing.T) {
	...
	mockProcessor := &mockProcessor{}
	mockProcessor.onTraces = func(ctx context.Context, received plog.Logs) error {
...
func TestAddSpansToExistingTrace(t *testing.T) {
	...
	var receivedTraces []plog.ResourceLogs
	next := &mockProcessor{
		onTraces: func(ctx context.Context, traces plog.Logs) error {
...
func TestTracesAreDispatchedInIndividualBatches(t *testing.T) {
	...
	st := newMemoryStorage()
	next := &mockProcessor{
		onTraces: func(_ context.Context, traces plog.Logs) error {
...
func BenchmarkConsumeTracesCompleteOnFirstBatch(b *testing.B) {
	...
	next := &mockProcessor{onTraces: func(context.Context, plog.Logs) error {
		<-semaphoreCh
		return nil
	}}
...
type mockProcessor struct {
	mutex    sync.Mutex
	onTraces func(context.Context, plog.Logs) error
}

var _ component.TracesProcessor = (*mockProcessor)(nil)

func (m *mockProcessor) ConsumeTraces(ctx context.Context, td plog.Logs) error {
	if m.onTraces != nil {
		m.mutex.Lock()
...
func (b *blockingConsumer) ConsumeTraces(context.Context, plog.Logs) error {
	<-b.blockCh
	return nil
}

func simpleTraces() plog.Logs {
	return simpleTracesWithID(pcommon.NewTraceID([16]byte{1, 2, 3, 4}))
}

func simpleTracesWithID(traceID pcommon.TraceID) plog.Logs {
	traces := plog.NewLogs()
