...
func TestEventCallback(t *testing.T) {
	for _, tt := range []struct {
	...
	}{
		{
			...
			typ:      traceReceived,
			payload:  tracesWithID{id: pcommon.InvalidTraceID(), td: plog.NewLogs()},
			registerCallback: func(em *eventMachine, wg *sync.WaitGroup) {
		...
		},
		{
			...
			registerCallback: func(em *eventMachine, wg *sync.WaitGroup) {
				em.onTraceReleased = func(expired []plog.ResourceLogs) error {
					wg.Done()
					return nil
				}
			},
...
func TestEventInvalidPayload(t *testing.T) {
	for _, tt := range []struct {
		...
		},
		{
			...
			typ:      traceReleased,
			registerCallback: func(em *eventMachine, wg *sync.WaitGroup) {
				em.onTraceReleased = func(released []plog.ResourceLogs) error {
					return nil
				}
			},
		},
